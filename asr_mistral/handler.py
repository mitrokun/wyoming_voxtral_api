...
class StreamAGC:
    ...
    def process(self, audio_chunk: np.ndarray) -> np.ndarray:
        ...
        if not self.is_calibrated:
            ...
            if (
                self.calib_frames <= 0
                or self.calib_peak > self.loud_threshold
            ):
        ...
        alpha = (
            ...
            else (0.0 if chunk_max < 0.005 else 0.002)
        )
        self.current_peak_envelope = (
            (1 - alpha) * self.current_peak_envelope + alpha * chunk_max
        )
        safe_envelope = max(self.current_peak_envelope, 1e-6)
        target_gain = self.target_level / safe_envelope
        final_gain = np.clip(
            target_gain, self.min_gain, self.active_max_gain
        )
        return np.tanh(audio_chunk * final_gain)

    def _finalize_calibration(self):
        self.is_calibrated = True
        self.active_max_gain = (
            1.0
...
class MistralEventHandler(AsyncEventHandler):
    ...
    async def handle_event(self, event: Event) -> bool:
        ...
        if AudioStart.is_type(event.type):
            ...
            self.mistral_task = asyncio.create_task(
                self._process_mistral_stream()
            )
        ...
        if AudioChunk.is_type(event.type):
            ...
            while len(self.internal_buffer) >= self.target_buffer_size:
                ...
                self.internal_buffer = self.internal_buffer[
        ...
        if AudioStop.is_type(event.type):
            ...
            if self.fast:
                _LOGGER.debug(
                    "AudioStop (FAST mode): waiting 150ms for partials..."
                )
    ...
    async def _flush_loop(self):
        ...
        try:
            while True:
                ...
                if self.unflushed_text:
                    text_to_send = "".join(self.unflushed_text)
                    self.unflushed_text.clear()

                    _LOGGER.debug("Δ: %r", text_to_send)
                    await self.write_event(
                        TranscriptChunk(text=text_to_send).event()
                    )
    ...
    async def _send_final_transcript(self):
        ...
        if self.unflushed_text:
            ...
            _LOGGER.debug("Δ (final): %r", text_to_send)
            await self.write_event(TranscriptChunk(text=text_to_send).event())

        final_text = "".join(self.accumulated_text).strip()

        delay = (
            (time.time() - self.stop_received_at) * 1000
            if self.stop_received_at
            else 0
        )
        _LOGGER.info(
            "Sending result (%s) at %.1f ms: '%s'",
            "FAST" if self.fast else "FULL",
            delay,
            final_text,
        )
    ...
    async def _process_mistral_stream(self):
        try:
            audio_format = AudioFormat(
                encoding="pcm_s16le", sample_rate=EXPECTED_SAMPLE_RATE
            )
            async for event in self.client.audio.realtime.transcribe_stream(
                ...
                if isinstance(event, TranscriptionStreamTextDelta):
                    ...
                    # all deltas from mistral
                    # _LOGGER.debug("Raw Δ: %r", event.text)
                    
                    if self.flush_interval_ms > 0:
                        self.unflushed_text.append(event.text)
                    else:
                        await self.write_event(
                            TranscriptChunk(text=event.text).event()
                        )
