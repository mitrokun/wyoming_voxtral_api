...
async def main() -> None:
    ...
    parser.add_argument(
        "--model", default="voxtral-mini-transcribe-realtime-2602"
    )
    ...
    parser.add_argument(
        "--fast",
        action="store_true",
        help="Send result early without waiting for server stream end",
    )
    parser.add_argument(
        "--ms",
        type=int,
        default=240,
        help="Chunk duration in ms (160, 240, 480, 960)",
    )
    parser.add_argument(
        "--flush-ms",
        type=int,
        default=150,
        help="Interval in ms to group and flush text chunks (0 to disable)",
    )
    ...
    _LOGGER.info(
        "Server starting on %s [Chunk size: %s ms, Fast mode: %s, Flush interval: %s ms]",
        args.uri,
        args.ms,
        args.fast,
        args.flush_ms,
